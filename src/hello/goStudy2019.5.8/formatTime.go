...
func main() {
	// 当前时间戳
	now := time.Now().Unix()
	nowNAm := time.Now().UnixNano()
	fmt.Println(now, nowNAm)
	// 当前格式化时间
	...
	str_time2 := time.Unix(1557304653, 0).Format("2006年01月02日 15时04分05秒")
	...
	str_time4 := time.Unix(1557304653, 0).Format("06-01-02 15:04:05") //-7 时区
	fmt.Println(str_time4)
	str_time5 := time.Unix(1557304653, 0).Format("01-02 15:04")
